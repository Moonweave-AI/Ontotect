...
该包没有依赖或生命周期脚本；仅获取包不会复制技能。只有获授权的组织 scoped npm 发布成功后，公共 `npx @moonweave-ai/ontotect` 命令才可用。源码、本地包、registry、用户级、覆盖及安全详情见 [npm 与 npx 安装](npm-and-npx-installation.md)。
