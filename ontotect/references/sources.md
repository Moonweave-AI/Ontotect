...
- Post-release verification of 0.1.0 on 2026-08-07 confirmed the initial public MIT package, anonymous registry access, organization read-write access, successful public npx help, and project-scoped installation into isolated Cursor, Codex, Kilo, OpenCode, and Claude Code layouts; at that time, `latest` pointed to `0.1.0`.
- Post-release verification on 2026-08-07 confirmed `@moonweave-ai/ontotect@0.1.1` as a public MIT package with exact version and repository metadata, the `ontotect` executable, `latest` set to `0.1.1`, and successful anonymous metadata access. Exact-version public npx help and isolated project-scoped installation into all five layouts passed with 48 files per target and directly identical byte content. The npm page rendered the centered brand mark, banner, badges, redesigned README sections, and Moonweave-AI repository link. Live-host loading and user/global-scope installation were not exercised.
- Post-release verification on 2026-08-07 confirmed `@moonweave-ai/ontotect@0.1.2` as a public MIT package authored by Moonweave AI, with exact version `0.1.2`, `latest` set to `0.1.2`, the `ontotect` executable, and Moonweave-AI repository metadata. Anonymous exact-version Help/List execution passed. An isolated project-scoped public-package installation produced all 20 registered skills for Cursor, Codex, Kilo, OpenCode, and Claude Code, plus all 20 command adapters for each of Kilo and OpenCode. The refreshed Codex runtime enumerated all 20 entries; the other four live host UIs and public-package user/global-scope installation were not exercised.
